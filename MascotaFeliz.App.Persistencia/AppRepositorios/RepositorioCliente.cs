...
namespace MascotaFeliz.App.Persistencia
{
    public class RepositorioCliente : IRepositorioCliente
    {
        ...
        {
            var clientencontrado = _appContext.Clientes.FirstOrDefault(c => c.Id == idPersona);
            if (clientencontrado == null) 
                return;
            _appContext.Clientes.Remove(clientencontrado);
            _appContext.SaveChanges();

        }
        Cliente IRepositorioCliente.GetCliente(int idPersona)
        {
            return _appContext.Clientes.FirstOrDefault(p => p.Id == idPersona);
        }
